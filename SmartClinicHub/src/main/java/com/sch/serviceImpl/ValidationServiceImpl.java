...
public class ValidationServiceImpl implements ValidationService {
	...
	@Override
	public Response<?> validateClientRegisterPayload(RegistrationDto registrationDto) {
		List<String> errors = new ArrayList<>();

		if (registrationDto == null) {
			errors.add("RegistrationDto should not be null");
			return new Response<>(HttpStatus.BAD_REQUEST.value(), "Validation Failed", errors);
		}

		if (registrationDto.getName() == null || registrationDto.getName().isEmpty()) {
			errors.add("Provide user name");
		}
		...
		if (registrationDto.getPhone() == null || registrationDto.getPhone().isEmpty()
				|| !registrationDto.getPhone().matches("^(\\+91)?\\d{10}$")) {
			errors.add("Provide valid phone number");
		}
	...
	public Response<?> validatePatientRegistrationPayload(PatientDto patientDto) {
		...
		if (patientDto.getPhone() == null || patientDto.getPhone().isEmpty()
				|| !patientDto.getPhone().matches("^(\\+91)?\\d{10}$")) {
